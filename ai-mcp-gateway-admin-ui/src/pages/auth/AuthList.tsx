...
export function AuthList() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center gap-3">
            <Input placeholder="网关ID" value={searchGatewayId} onChange={(e) => setSearchGatewayId(e.target.value)} className="max-w-[220px]" onKeyDown={(e) => e.key === 'Enter' && handleSearch()} />
            <Button variant="outline" onClick={handleSearch}><Search className="h-4 w-4 mr-2" />搜索</Button>
            <Button variant="ghost" onClick={() => { setSearchGatewayId(''); setPage(1); }}><RefreshCw className="h-4 w-4 mr-2" />重置</Button>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base"><Shield className="h-4 w-4 inline mr-2" />认证列表 ({total} 条记录)</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-muted-foreground"><Shield className="h-10 w-10 mb-3 opacity-30" /><p>暂无认证配置数据</p></div>
          ) : (
            <>
                  ...
                  <tbody>
                    {data.map((item) => (
                      <tr key={item.gatewayId} className="border-b hover:bg-slate-50 transition-colors">
                        ...
                        <td className="py-3 px-4 font-mono text-xs max-w-[300px] truncate" title={item.apiKey}>{item.apiKey}</td>
                        <td className="py-3 px-4">
                          <Badge variant={item.rateLimit > 0 ? 'default' : 'secondary'}>{item.rateLimit || '无限制'}</Badge>
                        </td>
                        <td className="py-3 px-4 text-xs text-muted-foreground">{formatDate(item.expireTime)}</td>
                        <td className="py-3 px-4 text-right">
                          <Button variant="ghost" size="sm" onClick={() => handleDelete(item.gatewayId)} className="text-red-500 hover:text-red-700"><Trash2 className="h-3 w-3" /></Button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent onClose={() => setDialogOpen(false)}>
          <DialogHeader><DialogTitle>新增认证配置</DialogTitle></DialogHeader>
